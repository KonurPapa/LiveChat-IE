...
comment.innerHTML = '<iframe id="postbox" style="width: 100%; height: 100%; box-shadow: 0 10px 10px 0 rgba(0, 0, 0, 0.1), 0 0 20px 0 rgba(0, 0, 0, 0.3);" frameborder="0" src="https://www.khanacademy.org/computer-programming/pjs/6144052898?editor=no"><div style="margin: 2%; padding: 2%; background-color: #f5f5f5;">If you can read this, then your browser does not support the built-in posting feature. You still have access to the chat, but may not have a good posting experience. For best results, please use the <a href="https://www.google.com/chrome/index.html" target="_blank">Google Chrome</a> browser.<br><br>Thanks for understanding, happy LiveChatting.</div></iframe>';

var apiUrl = "https://www.khanacademy.org/api/internal/discussions/scratchpad/6144052898/comments?casing=camel&limit=500&sort=2";
...
//window.setInterval(function() {
    $.getJSON(apiUrl + "&callback=?", function(data) {
        ...
        for (var i = 0; i < data.feedback.length; i++) {
            ...
            $("[meta-area]").append(
                $("<img src='" + uniqueImgs[i - 2] + "' width='9%' style='margin-right: 1%; margin-top: -2%;' class='" + uniqueNames[i - 2] + "'>")
            ).append(
                $("<a href='https://www.khanacademy.org/profile/" + uniqueIds[i - 2] + "' target='_blank' class='" + uniqueNames[i - 2] + "'>").text(uniqueNames[i - 2])
            ).append(
                $("<img src='" + img + "' width='12%' style='position: absolute; left: 87%; margin-top: -1%; border: 1px solid #e9e9e9; border-radius: 1px;' class='" + uniqueNames[i - 2] + "'>")
            ).append(
                $("<hr class='" + uniqueNames[i - 2] + "'>").css("margin", "5%")
            )
