...
function getChat(link) {
    ...
    function format(msg) {
        ...
        var blacklist = [
            /\x20[\x61\x2D\x2A]\x73\x73\x20/gi,
            /\x20[\x61\x2D\x2A]\x72\x73\x65\x20/gi,
    ...
    var repeat = setInterval(function() {
        $.getJSON("https://www.khanacademy.org/api/internal/discussions/scratchpad/" + link + "/comments?sort=2&callback=?", function(data) {
            $("[data-area]").empty();
            $("[meta-area]").empty();
